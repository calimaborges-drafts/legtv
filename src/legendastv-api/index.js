...
const downloadFromLegendasTv = async (authCookie, id) => {
  ...
  if (fileBuffer.toString() === "(null)") return null;
...
const listUnpackedFiles = async (authCookie, ids) => {
  return await Promise.reduce(
    ids,
    async (listOfSubtitles, id) => {
      const filePath = await downloadFromLegendasTv(authCookie, id);
      if (filePath === null) return listOfSubtitles;
...
const querySubtitlesFromLegendasTv = async (username, password, query) => {
  ...
  return listUnpackedFiles(authCookie, topLinks);
};
